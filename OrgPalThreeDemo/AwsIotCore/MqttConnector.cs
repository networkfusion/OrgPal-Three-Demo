...
namespace OrgPalThreeDemo.AwsIotCore
{
    public static class MqttConnector
    {
        ...
        {
            if (string.IsNullOrEmpty(AwsIotCore.MqttConnector.PrimaryConnectionCertificates.ClientRsaSha256Crt))
            {
                return false;
            }
            if (AwsIotCore.MqttConnector.PrimaryConnectionCertificates.RootCA.Length < 100) // we will presume that it has to be greater...
            {
                return false;
            }
            if (string.IsNullOrEmpty(AwsIotCore.MqttConnector.PrimaryConnectionCertificates.ClientRsaKey))
            {
                return false;
            }
