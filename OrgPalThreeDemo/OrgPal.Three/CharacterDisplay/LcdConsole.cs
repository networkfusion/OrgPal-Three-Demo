...
namespace Iot.Device.CharacterLcd
{
    ...
    public sealed class LcdConsole : IDisposable
    {
        ...
        public void Write(string text)
        {
            ...
            //TODO: this should(needs) to work!
            ...
            {
                if (linesSplit[i].EndsWith("\r\n"))
                {
                    linesSplit[i] = linesSplit[i].Substring(0, linesSplit[i].Length - 2) + "\n";
                }

                lines.Add(linesSplit[i]);
            }
        ...
        private void BlinkDisplayAsync(int times)
        {
            new Thread(() =>
            {
        ...
        private void WriteCurrentLine(string line)
        {
            ...
            _currentData[CursorTop].Insert(CursorLeft, line, line.Length);
